...
-- create a new user
CREATE USER cell_owner IDENTIFIED BY &1;

-- allow the user to connect to the database
grant connect to cell_owner;

-- provide quota on all tablespaces with BPM tables
grant unlimited tablespace to cell_owner;

-- grant privileges to create database objects:
grant resource to cell_owner;
grant create view to cell_owner;

-- grant access rights to resolve lock issues
grant execute on dbms_lock to cell_owner;

-- grant access rights to resolve XA related issues:
grant select on pending_trans$ to cell_owner;
grant select on dba_2pc_pending to cell_owner;
grant select on dba_pending_transactions to cell_owner;
-- If using Oracle 10.2.0.3 or lower JDBC driver, un-comment the following statement:
-- grant execute on dbms_system to cell_owner; 
-- If not using Oracle 10.2.0.4 or higher JDBC driver, comment the following statement:
grant execute on dbms_xa to cell_owner;
